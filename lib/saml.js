...
const common_1 = require("./saml/common");
...
class SAML {
    ...
    initialize(ctorOptions) {
        ...
        const options = {
            ...
            cacheProvider: (_t = ctorOptions.cacheProvider) !== null && _t !== void 0 ? _t : new inmemory_cache_provider_1.InMemoryCacheProvider({
            ...
            generateUniqueId: (_y = ctorOptions.generateUniqueId) !== null && _y !== void 0 ? _y : crypto_1.generateUniqueId,
            racComparison: (_z = ctorOptions.racComparison) !== null && _z !== void 0 ? _z : "exact",
        };
        /**
    ...
    signRequest(samlMessage) {
        ...
        samlMessage.Signature = signer.sign(crypto_1.keyToPEM(this.options.privateKey), "base64");
    }
    async generateAuthorizeRequestAsync(isPassive, isHttpPostBinding, host) {
        ...
        const instant = datetime_1.generateInstant();
        if (![false, types_1.ValidateInResponseTo.never].includes(this.options.validateInResponseTo)) {
    ...
    async _requestToUrlAsync(request, response, operation, additionalParameters) {
        ...
        if (types_1.isValidSamlSigningOptions(this.options)) {
    ...
    _getAdditionalParams(relayState, operation, overrideParams) {
        return common_1.getAdditionalParams({
            relayState,
            globalAdditionalParams: this.options.additionalParams,
            operationAdditionalParams: operation === "logout"
    ...
    validateSignature(fullXml, currentNode, certs) {
        ...
        return certs.some((certToCheck) => {
            return xml_1.validateXmlSignatureForCert(signature, crypto_1.certToPEM(certToCheck), fullXml, currentNode);
        });
    }
    ...
    async validateInResponseTo(inResponseTo) {
        if (![false, types_1.ValidateInResponseTo.never].includes(this.options.validateInResponseTo)) {
            ...
            else if ([true, types_1.ValidateInResponseTo.always].includes(this.options.validateInResponseTo)) {
    ...
    validateSignatureForRedirect(urlString, signature, alg, cert) {
        ...
        return verifier.verify(crypto_1.certToPEM(cert), signature, "base64");
    }
    ...
    async processValidlySignedAssertionAsync(xml, samlResponseXml, inResponseTo) {
        ...
        getInResponseTo: {
            ...
            if ([true, types_1.ValidateInResponseTo.always, types_1.ValidateInResponseTo.ifPresent].includes(this.options.validateInResponseTo)) {
                if (subjectConfirmation) {
                    if (confirmData && confirmData.$) {
                        const subjectInResponseTo = confirmData.$.InResponseTo;
                        if (!inResponseTo &&
                            this.options.validateInResponseTo === types_1.ValidateInResponseTo.ifPresent) {
    ...
    checkTimestampsValidityError(nowMs, notBefore, notOnOrAfter, maxTimeLimitMs) {
        if (this.options.acceptedClockSkewMs == -1)
            return null;
        if (notBefore) {
            const notBeforeMs = datetime_1.dateStringToTimestamp(notBefore, "NotBefore");
            if (nowMs + this.options.acceptedClockSkewMs < notBeforeMs)
                return new Error("SAML assertion not yet valid");
        }
        if (notOnOrAfter) {
            const notOnOrAfterMs = datetime_1.dateStringToTimestamp(notOnOrAfter, "NotOnOrAfter");
    ...
    generateServiceProviderMetadata(decryptionCert, signingCert) {
        ...
        if (this.options.decryptionPvk != null || this.options.privateKey != null) {
            ...
            if (types_1.isValidSamlSigningOptions(this.options)) {
                signingCert = utility_1.assertRequired(signingCert, "Missing signingCert while generating metadata for signing service provider messages");
                ...
                const signingKeyDescriptors = certArray.map((cert) => ({
                    ...
                    "ds:KeyInfo": {
                        "ds:X509Data": {
                            "ds:X509Certificate": {
                                "#text": crypto_1.removeCertPEMHeaderAndFooter(cert),
                            },
                        },
                    },
                }));
            ...
            if (this.options.decryptionPvk != null) {
                decryptionCert = utility_1.assertRequired(decryptionCert, "Missing decryptionCert while generating metadata for decrypting service provider");
                decryptionCert = crypto_1.removeCertPEMHeaderAndFooter(decryptionCert);
        ...
        let metadataXml = xml_1.buildXmlBuilderObject(metadata, true);
        if (this.options.signMetadata === true && types_1.isValidSamlSigningOptions(this.options)) {
            metadataXml = utility_1.signXmlMetadata(metadataXml, this.options);
        }
        return metadataXml;
    }
    /**
    ...
    processMaxAgeAssertionTime(maxAssertionAgeMs, notOnOrAfter, issueInstant) {
        const notOnOrAfterMs = datetime_1.dateStringToTimestamp(notOnOrAfter, "NotOnOrAfter");
        const issueInstantMs = datetime_1.dateStringToTimestamp(issueInstant, "IssueInstant");
